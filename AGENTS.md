...
| File | Responsibility |
|------|---------------|
| `sse.js` | Reading a round's SSE stream from a `fetch` response (copied from `chat.js`); `splitSSE()` is the pure splitter |
| `player.js` | The show's voice: `chunks()` cuts a line into whole-sentence chunks (the accumulator: up to 100 characters, a short sentence riding along to 120, never across lines); the chunks are synthesized through `/api/tts` one at a time and played in order; `drained()`, `stopVoice()` (copied from `tts.js` without the chat's audio upload) |
...
| `show.js` | The show's states and loop: start a run, play its rounds (each line given to the voice and shown when it starts; the next round when the voice has drained, with the seconds played; `?voice=off` plays text only on a simulated clock), the listener's turn after an invitation (the window, hold to talk up to the press cap, what Whisper heard in the next round request, and shown at once as a "You: …" caption with each word marked by Whisper's confidence, plus the filter's verdict when it counted as silence), Stop / Resume, captions, the debug line |
...
| `POST` | `/api/show/start` | Open a show run of a story (default `show.story`); returns `{run_id, story, title, cast, operator, seed, listen_window_s, press_cap_s, debug}` — the last three for the show page (the listener's timers, the debug line) |
...
| `POST` | `/api/show/listen` | Transcribe a show listener's recording with the show's Whisper settings (the cast's names as `prompt`, `language`, `vad_filter`); returns `{text, no_speech_prob, avg_logprob, words}` — the first three for the next round request, `words` (each word with Whisper's probability) for the page's caption only; 503 when STT is inactive, 502 when it fails |
